    ...
    return {
        ...
        initSlider(sliderElement) {
            ...
            for (let indicatorElement of sliderElement.querySelectorAll(".slider-indicators")) {
                indicatorElement.addEventListener("click", (event) => this.slideToByIndicator(event));
            ...
            if (sliderElement.classList.contains("slider-nav-autoplay")) {
                const timeout = sliderElement.getAttribute("data-slider-nav-autoplay-interval") || 2500;
                this.autoPlay(sliderElement, timeout, sliderElement.classList.contains("slider-nav-autopause"));
            }
            if (["slider-nav-autohide", "slider-nav-animation"].some(className => sliderElement.classList.contains(className))) {
                const threshold = parseFloat(sliderElement.getAttribute("data-slider-nav-animation-threshold")) || 0.5;
                this.setVisibleSlides(sliderElement, threshold);
            }
        },

        setVisibleSlides(sliderElement, threshold = 0.5) {
            let observer = new IntersectionObserver(slides => {
                slides.forEach(slide => {
                    slide.isIntersecting ? slide.target.classList.add("slide-visible") : slide.target.classList.remove("slide-visible");
                });
                const firstChild = sliderElement.querySelector(".slider-container>*:first-child");
                const lastChild = sliderElement.querySelector(".slider-container>*:last-child");
                if (firstChild && lastChild) {
                    firstChild.classList.contains("slide-visible") ? sliderElement.classList.add("slider-item-first-visible") : sliderElement.classList.remove("slider-item-first-visible");
                    lastChild.classList.contains("slide-visible") ? sliderElement.classList.add("slider-item-last-visible") : sliderElement.classList.remove("slider-item-last-visible");
                }
            }, {
        ...
        slide(sliderElement, next = true) {
            ...
            const scrollStep = slides[0]?.offsetWidth + gapWidth || 0;
            let scrollLeftPosition = next ? container.scrollLeft + scrollStep : container.scrollLeft - scrollStep;
            
            if (next && container.scrollLeft >= (container.scrollWidth - container.offsetWidth - 1)) {
        ...
        slideTo(sliderElement, slideIndex) {
            const container = sliderElement.querySelector(".slider-container");
            const gapWidth = parseInt(window.getComputedStyle(container).columnGap) || 0;
            const scrollStep = container.children[0]?.offsetWidth + gapWidth || 0;
            container.scroll({
        ...
        autoPlay(sliderElement, timeout, autopause) {
            timeout = Math.max(timeout, 750);
            if (sliderElement.autoplayTimer) clearInterval(sliderElement.autoplayTimer);
            sliderElement.autoplayTimer = setInterval(() => this.slide(sliderElement), timeout);
            if (autopause) {
                ["mouseover", "touchstart"].forEach(event => {
                    sliderElement.addEventListener(event, () => clearInterval(sliderElement.autoplayTimer), { once: true, passive: true });
                });
                ["mouseout", "touchend"].forEach(event => {
                    sliderElement.addEventListener(event, () => this.autoPlay(sliderElement, timeout, autopause), { once: true, passive: true });
                });
            }
        },
        ...
        handleIndicators(sliderElement) {
            ...
            for (let scrollIndicatorContainers of sliderElement.querySelectorAll(".slider-indicators")) {
                ...
                if (scrollIndicators[activeIndicator]) scrollIndicators[activeIndicator].classList.add("active");
            }
        }
    };
}();
